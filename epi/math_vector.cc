...
static inline float epi_sqrtf(float v) { return fitd_sqrtf(v); }
#else
static inline float epi_sqrtf(float v) { return sqrtf(v); }
#endif
...
{
	return epi_sqrtf(x * x + y * y + z * z);
}

float vec3_c::Slope() const
{
	float dist = epi_sqrtf(x * x + y * y);
...
{
	return epi_sqrtf(x * x + y * y + z * z + w * w);
}
