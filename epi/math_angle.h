...
// NOTE: EPI already uses FitdLib's Dreamcast fastmath helpers for SH-4 acceleration.
// Keep this include gated to avoid coupling desktop builds unnecessarily.
...
public:
	...
	inline void SinCos(float *outSin, float *outCos) const
	{
	#if defined(DREAMCAST) && DITD_ENABLE_EPI_SH4_ACCEL
		// angle_c is BAM (2^32 units per turn). SH-4 FSCA expects 2^16 units per turn.
		fitd_fsca_u16(bam >> 16, outSin, outCos);
	#else
		const float rad = (float)Radians();
		if (outSin) *outSin = sinf(rad);
		if (outCos) *outCos = cosf(rad);
	#endif
	}

	inline float Sin() const { float s; SinCos(&s, nullptr); return s; }
	inline float Cos() const { float c; SinCos(nullptr, &c); return c; }
	inline float Tan() const { return tanf((float)Radians()); }
...
{
	return FromRadians((double)atanf(slope));
}
